...
void radial_menu_controller_add_new_tab(AppData *app_data) {
    ...
    g_signal_connect(key_controller, "key-pressed", G_CALLBACK(radial_menu_controller_handle_key), app_data);
}
...
gboolean radial_menu_controller_handle_key(GtkEventControllerKey *controller,
    ...
    VteTerminal *terminal = VTE_TERMINAL(gtk_event_controller_get_widget(GTK_EVENT_CONTROLLER(controller)));
